...
export const adminRouter = createTRPCRouter({
  stats: adminProcedure.query(async ({ ctx }) => {
    const [totalUsers, totalListings, openReports, totalSales] = await Promise.all([
      ctx.db.user.count(),
  ...
  getReports: adminProcedure
    .input(z.object({ status: z.enum(["OPEN", "UNDER_REVIEW", "RESOLVED", "DISMISSED"]).optional(), limit: z.number().min(1).max(100).default(50) }))
  ...
  updateReport: adminProcedure
    .input(z.object({ reportId: z.string(), status: z.enum(["OPEN", "UNDER_REVIEW", "RESOLVED", "DISMISSED"]) }))
    .mutation(async ({ ctx, input }) => {
      return ctx.db.report.update({ where: { id: input.reportId }, data: { status: input.status } });
    }),
  getUsers: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return ctx.db.user.findMany({
        where, select: { id: true, name: true, email: true, verified: true, role: true, createdAt: true, _count: { select: { listings: true, purchases: true, sales: true } } },
  ...
  removeListing: adminProcedure.input(z.object({ listingId: z.string() })).mutation(async ({ ctx, input }) => {
