...
export const courseRouter = createTRPCRouter({
  ...
  removeCourse: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.courseSchedule.deleteMany({
  ...
  myCourses: protectedProcedure.query(async ({ ctx }) => {
    return ctx.db.courseSchedule.findMany({
  ...
  suggestedBooks: protectedProcedure.query(async ({ ctx }) => {
    const courses = await ctx.db.courseSchedule.findMany({
      where: { userId: ctx.userId },
  ...
  popularCourses: publicProcedure.query(async ({ ctx }) => {
    const listings = await ctx.db.listing.groupBy({
      by: ["courseCode"],
      where: { courseCode: { not: null }, status: "AVAILABLE" },
      _count: { id: true },
      orderBy: { _count: { id: "desc" } },
      take: 20,
    });
    return listings
      .filter((l) => l.courseCode)
      .map((l) => ({ courseCode: l.courseCode!, count: l._count.id }));
  }),
});
