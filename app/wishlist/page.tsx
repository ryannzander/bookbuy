...
export default function WishlistPage() {
  ...
  const toggle = api.wishlist.toggle.useMutation({
    onSuccess: () => utils.wishlist.getAll.invalidate(),
  });
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Saved Books</h1>
          <p className="text-muted-foreground">Books you&apos;re interested in</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-border bg-card p-12 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            Browse the marketplace and save books you&apos;re interested in.
          </p>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {data.map((item) => (
            <div
              key={item.id}
              className="rounded-2xl border border-border bg-card p-5 flex gap-4 hover:border-muted-foreground/30 transition-colors"
            >
              <div className="h-20 w-16 rounded-lg bg-secondary flex items-center justify-center shrink-0">
              ...
              <button
                onClick={() => toggle.mutate({ listingId: item.listing.id })}
                className="self-start p-2 rounded-lg hover:bg-secondary transition-colors text-muted-foreground hover:text-destructive"
              >
