...
export default function AdminPage() {
  ...
  const isAdmin = me && (me as { role?: string }).role === "ADMIN";
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Moderate content and manage users. All actions are logged.</p>
        </div>
      </div>
            ...
            <div key={report.id} className="rounded-xl border border-border p-4 space-y-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground mt-2 space-x-3">
                  ...
                  {report.targetUser && (
                    <span>Target: {report.targetUser.name ?? report.targetUser.email}</span>
                  )}
              ...
              <div className="flex gap-2 flex-wrap">
                {report.status === "OPEN" && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => updateReport.mutate({ reportId: report.id, status: "UNDER_REVIEW" })}
                  >
                ...
                {(report.status === "OPEN" || report.status === "UNDER_REVIEW") && (
                  <>
                    <Button
                      size="sm"
                      onClick={() =>
                        updateReport.mutate({ reportId: report.id, status: "RESOLVED" })
                      }
                    >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        updateReport.mutate({ reportId: report.id, status: "DISMISSED" })
                      }
                    >
                ...
                {report.listing && (
                  <Button
                    ...
                    variant="destructive"
                    onClick={() => {
                      if (confirm("Remove this listing? Seller will be notified.")) {
                ...
                {report.targetUser && report.targetUser.id !== me.id && (
                  <Button
                    size="sm"
                    variant="destructive"
                    onClick={() => {
                      if (confirm("Restrict this user? They will be notified.")) {
                        banUser.mutate({ userId: report.targetUser!.id });
                      }
                    }}
                  >
                    <AlertTriangle className="h-3.5 w-3.5 mr-1" /> Ban
                  </Button>
                )}
              </div>
            </div>
